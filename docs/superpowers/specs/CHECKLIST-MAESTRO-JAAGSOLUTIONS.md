...
- [ ] **Calidad visual — evitar uncanny valley en personajes** — el prompt actual puede generar figuras humanas con piel plástica/artificial. Solución: (A) agregar `plastic skin, uncanny valley, doll-like, cgi face` al `negative_prompt` del Code node Ideogram; (B) agregar instrucción en el spec de A4 para preferir composiciones sin rostros en primer plano (pantallas, manos, workspaces abstractos). Prioridad B como cambio de spec, A como cambio en workflow JSON.
