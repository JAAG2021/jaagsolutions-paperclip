...
- [x] **Calidad visual — evitar uncanny valley en personajes** — ✅ Completado 2026-05-17. (A) `negative_prompt` actualizado en Code node `Ideogram + OCR — 3 intentos`: agregados `plastic skin, uncanny valley, doll-like, cgi face, artificial skin, deformed face, distorted proportions, wax figure, overly smooth skin, lifeless eyes`. (B) Spec de A4 actualizado en `jaagsolutions-seed.json`: nueva sección `COMPOSICION VISUAL (image_prompt para pipeline automatizado)` con instrucciones explícitas de priorizar pantallas/manos/workspaces/flujos visuales y evitar "persona mirando a cámara". Requiere transplant del workflow + docker cp + re-seed (ver comandos abajo).
