    ...
    <section id="proceso" className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative grid md:grid-cols-3 gap-8 mb-12">
          ...
          <div className="hidden md:block absolute top-16 left-[calc(16.67%+1rem)] right-[calc(16.67%+1rem)] h-0.5 bg-gradient-to-r from-brand-200 via-violet-200 to-emerald-200 z-0" />

          {steps.map((step, i) => (
            <div
              key={step.number}
              className={`card-hover animate-fade-in-up-d${i + 1} relative z-10 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden`}
            >
              {/* Colored top bar */}
              ...
              <div className="p-7">
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${step.color} flex items-center justify-center text-white font-extrabold text-sm shadow-md`}>
                    {step.number}
                  </div>
